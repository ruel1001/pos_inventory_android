...
@AndroidEntryPoint
class CustomerSearchNamesFragment : Fragment(), CustomerNamesAdapter.ArticleCallback {
    private var _binding: FragmentCustomerSaerchNameBinding? = null
    private val binding get() = _binding!!
    private val viewModel: CreateCustomerViewModel by viewModels()
    private var loadingDialog: CommonDialog? = null
    private var linearLayoutManager: LinearLayoutManager? = null
    private var customerNamesAdapter: CustomerNamesAdapter? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentCustomerSaerchNameBinding.inflate(
            inflater,
            container,
            false
        )
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.searchNameEditText.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                ...
                viewModel.getCustomerNameLIst(newText)
            }
        })
    ...
    private fun setupPaymentList() = binding.run {
        ...
        customerNamesAdapter =
            CustomerNamesAdapter(requireActivity(), this@CustomerSearchNamesFragment)
        linearLayoutManager = LinearLayoutManager(context)
        nameslistRecyclerView.layoutManager = linearLayoutManager
        nameslistRecyclerView.adapter = customerNamesAdapter
        setupScrollListener()
    }
        ...
        linearLayoutManager?.let {
            scrollListener = object : EndlessRecyclerScrollListener(it) {
                override fun onLoadMore(page: Int, totalItemsCount: Int) {
                    viewModel.getCustomerNameLIst(binding.searchNameEditText.text.toString())
                }
            }
        }
        scrollListener?.let {
    ...
    private fun observeArticleList() {
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.customerSharedFlow.collect { viewState ->
    ...
    private fun handleViewState(viewState: CreateCustomerViewState) {
        when (viewState) {
            is CreateCustomerViewState.Loading -> Unit
            is CreateCustomerViewState.SuccessListNames -> {
                customerNamesAdapter?.clear()
                customerNamesAdapter?.appendData(viewState.response?.data.orEmpty())
                binding?.collectible?.text = viewState?.response?.account_balance
            }
    ...
    companion object {
        ...
        fun getIntent(context: Context): Intent {
            return Intent(context, CustomerSearchNamesFragment::class.java)
        }
    ...
    override fun onItemClicked(data: SearchListNameData) {
