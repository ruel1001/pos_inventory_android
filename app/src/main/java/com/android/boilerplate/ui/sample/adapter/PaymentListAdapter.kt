...
import com.android.boilerplate.data.repositories.payment.response.PaymentListResponse.PaymentListData
...
class PaymentListAdapter(val context: Context, val clickListener: ArticleCallback) :
    RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val TYPE_HEADER = 0
    private val TYPE_ROW = 1
    private val adapterData = mutableListOf<PaymentListData>()
    ...
    fun appendData(newData: List<PaymentListData>) {
        val startAt = adapterData.size
        adapterData.addAll(newData)
        notifyItemRangeInserted(startAt, newData.size)
    }

    fun getData(): MutableList<PaymentListData> = adapterData
    ...
    inner class HeaderViewHolder(val binding: AdapterPaymentHeaderLayoutBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bindHeader() {
            ...
            binding.header2TextView.text="Amount Paid"
            binding.header3TextView.text="Billing Month"
    ...
    inner class RowViewHolder(val binding: AdapterRowPaymentLayoutBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bindRow(data: PaymentListData) {
            // Bind row data
    ...
    interface ArticleCallback {
        fun onItemClicked(data: PaymentListData)
    }
}